...
interface CalendarBlock { id: string; title: string; date_planned: string; estimated_duration_minutes: number; status: string; technician_id: string | null; intervention_type?: string | null; }
...
function CreateInterventionSplitView({ onSuccess, onCancel }: { onSuccess: () => void; onCancel: () => void }) {
  ...
  const fetchCalendarData = useCallback(async () => {
    let q = supabase.from('interventions').select('id, title, date_planned, estimated_duration_minutes, status, technician_id, intervention_type').gte('date_planned', weekStart.toISOString()).lte('date_planned', weekEnd.toISOString()).not('status', 'eq', 'annule');
